...
logger = utils.get_logger(logpath=os.path.join('./data', 'logs'), filepath=os.path.abspath(__file__))
logger.info(args)
...
def run(args, kwargs):
    ...
    if not args.evaluate:
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            v_loss, v_bpd = evaluate(test_loader, model, args, logger, epoch=epoch)
            ...
            if v_loss < best_loss:
                ...
                torch.save(model, snap_dir + args.flow + '{}.model'.format(checkpoint_id))
        ...
        final_model = torch.load(snap_dir + args.flow + '{}.model'.format(checkpoint_id))
    ...
    if args.wandb_name:
        wandb.log({"VALIDATION_LOSS": validation_loss, "TEST_LOSS": test_loss})
        if args.input_type != 'binary':
            wandb.log({"VALIDATION_BPD": validation_bpd, "TEST_BPD": test_bpd})
