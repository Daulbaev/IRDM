...
SOLVERS = ["dopri5", 'dopri5_old', "bdf", "rk4", "midpoint", 'adams', 'explicit_adams', 'fixed_adams']
...
parser.add_argument('--odeint_type', type=str, default='adjoint_chebyshev',
                    choices=['adjoint', 'adjoint_chebyshev', 'adjoint_linear'])
...
checkpoint_id = str(time.time()).split('.')[-1]
if args.wandb_name:
...
def get_dt(cnfs):
    return {idx: cnf.dt for idx, cnf in enumerate(cnfs)}


def set_dt(cnfs, value):
    for cnf in cnfs:
        cnf.dt = deepcopy(value)
    return cnfs


def get_nbe(cnfs):
    return {idx: cnf.nbe for idx, cnf in enumerate(cnfs)}


def set_nbe(cnfs, value):
    for cnf in cnfs:
        cnf.nbe = deepcopy(value)
    return cnfs


def get_forward_t(cnfs):
    return {idx: cnf.forward_t for idx, cnf in enumerate(cnfs)}


def set_forward_t(cnfs, value):
    for cnf in cnfs:
        cnf.forward_t = deepcopy(value)
    return cnfs


def get_backward_t(cnfs):
    return {idx: cnf.backward_t for idx, cnf in enumerate(cnfs)}


def set_backward_t(cnfs, value):
    for cnf in cnfs:
        cnf.backward_t = deepcopy(value)
    return cnfs


def get_f_t(cnfs):
    return {idx: cnf.f_t for idx, cnf in enumerate(cnfs)}


def get_z_t(cnfs):
    return {idx: cnf.z_t for idx, cnf in enumerate(cnfs)}
...
if __name__ == '__main__':
    ...
    model, cnfs = build_model_tabular(args,
                                      data.n_dims,
                                      regularization_fns,
        ...
        while True:
            ...
            for x in batch_iter(data.trn.x, shuffle=True):
                if args.early_stopping > 0 and n_vals_without_improvement > args.early_stopping:
                    break
                cnfs = set_nbe(cnfs, 0)
                cnfs = set_forward_t(cnfs, list())
                cnfs = set_backward_t(cnfs, list())
                cnfs = set_dt(cnfs, list())
                ...
                if itr % args.log_freq == 0:
                    ...
                    #     'NFE Forward {:.0f}({:.1f}) | NFE Backward {:.0f}({:.1f}) | fTime {:} | bTime {:} | CNF '
                    #     'Time {:.4f}({:.4f}) | '
                    #     'FdT {:} | BdT {:} | NBE {:} | f(t) {:} | z(t) {:}'.format(
                    #         itr,
                    ...
                    #         nfeb_meter.avg, str(get_forward_t(cnfs)), str(get_backward_t(cnfs)), tt_meter.val, tt_meter.avg,
                    #         str(forward_dt), str(backward_dt), str(get_nbe(cnfs)),
                    #         str(f_t), str(z_t),
                    #     )
                    # )
                    if args.wandb_name:
                    ...
                    # }, os.path.join(args.save, "checkpt_{:d}.pth".format(itr)))
                itr += 1
                cnfs = set_nbe(cnfs, 0)
                cnfs = set_forward_t(cnfs, list())
                cnfs = set_backward_t(cnfs, list())
                ...
                if itr % args.val_freq == 0:
                    ...
                    with torch.no_grad():
                        ...
                        if val_loss.avg < best_loss:
                            ...
                            if args.wandb_name:
                                wandb.save(os.path.join(args.save, 'checkpt_{}.pth'.format(checkpoint_id)))
                            n_vals_without_improvement = 0
                        else:
                        ...
                        # )
                        if args.wandb_name:
                            wandb.log({'test_loss': val_loss.avg, 'test_iter': itr})
                        # logger.info(log_message)
                    model.train()

                end = time.time()

        model = restore_model(model, os.path.join(args.save, 'checkpt_{}.pth'.format(checkpoint_id))).to(device)
        set_cnf_options(args, model)
    ...
    with torch.no_grad():
        ...
        # logger.info(log_message)
        if args.wandb_name:
            wandb.log({'TEST_LOSS': test_loss.avg, 'TEST_NFE_AVG': test_nfe.avg})
