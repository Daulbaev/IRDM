...
def odesolver(func, z0, options=None, return_intermediate_points=False):
    if options is None:
        Nt = 2
    else:
        Nt = options['Nt']
    ...
    if hasattr(func, 'base_func'):
        container = func.base_func

    if hasattr(container, 'dt'):
        container.dt.append(solver.dt_next)

    if hasattr(container, 'z_t') and return_intermediate_points:
        container.z_t.append(solver.z_t)
