...
class Checkpointing_Adjoint(torch.autograd.Function):
    ...
    def forward(ctx, *args):
        ...
        with torch.no_grad():
            if 'return_inter_points' in options:
                return_inter_points = options['return_inter_points']
            else:
                return_inter_points = True
            ans = odesolver(func, z0, options, return_intermediate_points=return_inter_points)
