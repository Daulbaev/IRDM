...
save="$save_root/classification_bn1-NormFree_resblock-NormFreeActFree_odeblock-NormFreeActFree/$network/${solver}_n${n_nodes}"
if [[ "$solver" == "dopri5" ]]
then
  save="${save}_tol${atol}"
fi
...
bash -c "cd  && CUDA_DEVICE_ORDER=PCI_BUS_ID CUDA_VISIBLE_DEVICES=$gpu python3 $train_file \
                ...
                --method $solver \
                --n_nodes $n_nodes\
                --normalization_resblock 'NormFree'\
                --normalization_odeblock 'NormFree'\
                --normalization_bn1 'NormFree'\
                --activation_resblock 'ActFree'\
                --activation_odeblock 'ActFree'
                "

# ## Example: nohup bash scripts/train_bn1-NormFree_resblock-NormFreeActFree_odeblock-NormFreeActFree.sh -g 2 -f configs/classification_resnet10_tol1e-3_lr1e-1_bs512.cfg -p configs/pathes.py -s Euler -n 8 -r 0&
