...
save="$save_root/classification/$network/${solver}_n${n_nodes}"
if [[ "$solver" == "dopri5" ]]
then
  save="${save}_tol${atol}"
fi
...
### Example: nohup bash scripts/train.sh -g 2 -f configs/classification_resnet10.cfg -p configs/pathes.py -s RK2 -n 10 -r 0&
