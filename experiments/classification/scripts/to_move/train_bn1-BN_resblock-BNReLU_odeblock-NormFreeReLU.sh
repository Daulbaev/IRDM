...
save="$save_root/classification_bn1-BN_resblock-BNReLU_odeblock-NormFreeReLU/$network/${solver}_n${n_nodes}"
if [[ "$solver" == "dopri5" ]]
then
  save="${save}_tol${atol}"
fi
...
### Example: nohup bash scripts/train_bn1-BN_resblock-BNReLU_odeblock-NormFreeReLU.sh  -g 2 -f configs/classification_resnet10_tol1e-3_lr1e-1_bs512.cfg -p configs/pathes.py -s Euler -n 8 -r 0&
