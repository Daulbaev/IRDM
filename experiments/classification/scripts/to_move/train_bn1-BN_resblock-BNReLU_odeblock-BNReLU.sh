...
save="$save_root/classification_bn1-BN_resblock-BNReLU_odeblock-BNReLU/$network/${solver}_n${n_nodes}"
if [[ "$solver" == "dopri5" ]]
then
  save="${save}_tol${atol}"
fi
...
bash -c "cd  && CUDA_DEVICE_ORDER=PCI_BUS_ID CUDA_VISIBLE_DEVICES=$gpu python3 $train_file \
                ...
                --normalization_odeblock 'BN'\
                --normalization_bn1 'BN'\
                --activation_resblock 'ReLU'\
                --activation_odeblock 'ReLU'
                "

# ## Example: nohup bash scripts/train_bn1-BN_resblock-BNReLU_odeblock-BNReLU.sh -g 7 -f configs/classification_resnet10_tol1e-3_lr1e-1_bs512.cfg -p configs/pathes.py -s Euler -n 8 -r 0&
