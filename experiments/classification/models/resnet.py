...
class ResNet(nn.Module):
    ...
    @property
    def z_t(self):
        if hasattr(self.ODEBlocks[0], "z_t"):
            return {idx: layer.z_t for idx, layer in enumerate(self.ODEBlocks)}
        else:
            return {idx: layer.odefunc.z_t for idx, layer in enumerate(self.ODEBlocks)}

    @z_t.setter
    def z_t(self, value):
        for layer in self.ODEBlocks:
            layer.z_t = deepcopy(value)

    @property
    def f_t(self):
        if hasattr(self.ODEBlocks[0], "f_t"):
            return {idx: layer.f_t for idx, layer in enumerate(self.ODEBlocks)}
        else:
            return {idx: layer.odefunc.f_t for idx, layer in enumerate(self.ODEBlocks)}
