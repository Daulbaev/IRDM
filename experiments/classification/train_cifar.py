...
from interpolated_torchdiffeq import odeint_chebyshev_func, odeint_linear_func
from interpolated_torchdiffeq import odeint_adjoint

parser = argparse.ArgumentParser()

parser.add_argument('--network', type=str, choices=['resnet34', 'resnet18', 'resnet10', 'resnet6', 'resnet4', \
                                                    'preresnet34', 'preresnet18', 'preresnet10', 'preresnet6',
                                                    'preresnet4', \
                                                    'sqnxt'], default='sqnxt')
...
parser.add_argument('--odeint_type', type=str, default='adjoint_chebyshev',
                    choices=['adjoint_chebyshev', 'adjoint', 'anode', 'adjoint_piecewise_linear'])
...
if args.wandb_name:
    import wandb
    wandb.init(project="interpolations_cifar", entity='talgat_team')
...
if args.odeint_type == 'adjoint_chebyshev':
    if args.method in ['dopri5', 'euler', 'midpoint', 'rk4', 'bosh3']:
        odesolver = partial(odeint_chebyshev_func, t=torch.tensor([0.0, 1.0]),
                            n_nodes=args.n_nodes, method=args.method)
elif args.odeint_type == 'adjoint_piecewise_linear':
    if args.method in ['dopri5', 'euler', 'midpoint', 'rk4', 'bosh3']:
        odesolver = partial(odeint_linear_func, t=torch.tensor([0.0, 1.0]),
                            n_nodes=args.n_nodes, method=args.method)
elif args.odeint_type == 'adjoint':
    if args.method in ['dopri5', 'euler', 'midpoint', 'rk4']:
        odesolver = partial(odeint_adjoint, t=torch.tensor([0.0, 1.0]), method=args.method)
elif args.odeint_type == 'anode':
    # Not sure that it works :(
    from anode import odesolver_adjoint as odesolver
else:
    raise ValueError('Wrong args.odeint_type!')
...
class ODEBlock(nn.Module):
    ...
    def __init__(self, odefunc):
        ...
        self.options = {}
        # self.options.update({'first_step': 1e-2})
...
def test(epoch):
    ...
    if args.wandb_name:
        wandb.log({'test_epoch': epoch,
                   'test_loss': test_loss / (100 * (idx + 1)),
                   'test_accuracy': correct / total,
                   'test_time_meter': time.time() - epoch_time,
                   'test_nfe': sum(net.module.nfe.values()),
                   'test_PeakMemory': torch.cuda.max_memory_allocated(device)})
