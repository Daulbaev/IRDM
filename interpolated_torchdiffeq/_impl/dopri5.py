...
class Dopri5Solver(AdaptiveStepsizeODESolver):
    ...
    def __init__(
        ...
        **unused_kwargs
        ...
        super(Dopri5Solver, self).__init__(func, y0, rtol, atol, **unused_kwargs)
    ...
    def before_integrate(self, t):
        ...
        if self.first_step is None:
            direction = 1. if t[0] < t[-1] else -1.
            first_step = _select_initial_step(self.func, t[0], self.y0, 4, self.rtol[0], self.atol[0], f0=f0, direction=direction).to(t)
        else:
    ...
    def _adaptive_dopri5_step(self, rk_state):
        ...
        (y1, f1, y1_error, k), inter_value, inter_time = _runge_kutta_step(self.func, y0, f0, t0, dt, tableau=_DORMAND_PRINCE_SHAMPINE_TABLEAU)

        self.f_t += [f0, f1]
        self.z_t += [y0, y1]
        self.intermediate_values += inter_value
        self.intermediate_time += inter_time
        ########################################################
        #                     Error Ratio                      #
        ########################################################
