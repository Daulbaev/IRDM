...
class OdeintAdjointMethod(torch.autograd.Function):
    ...
    def forward(ctx, *args):
        ...
        with torch.no_grad():
            result = odeint(func, y0, t, rtol=rtol, atol=atol, method=method, options=options, context=ctx,
                            return_intermediate_points=return_intermediate)
            if return_intermediate:
                ans, inter_values, inter_times, z_t, f_t = result
            else:
                ans = result
...
def odeint_adjoint(func,
    ...
    ys = OdeintAdjointMethod.apply(*y0,
                                   func,
                                   t,
                                   flat_params,
                                   rtol,
